_pfreak.tasks.push({
    ...
    short_name: "remove_single_class_string_from_multiple",
    display_order: 53,
    category: "styling",
    description: "",
    assert_delay: 0,
    ...
    candidates: {
        "caldom": function(config){
            return config.caldomInstance.removeClass("second-class");
        }
    },
    ...
    assert: function(config, test_return){
        ...
        if( inputs[0].className != "first-class third-class" ) throw false;
        if( inputs[1].className != "first-class third-class" ) throw false;
    },
