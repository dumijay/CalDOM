_pfreak.tasks.push({
    ...
    short_name: "deleting_500_of_1000_rows",

    category: "mixed_reactive",
    description: 'Deleting 500 rows of 1000 rows with new data, each containing 1 text cell with a click event and a background color.',
    assert_delay: 1000,
    ...
    setTaskData: function(config){
        ...
        for( var i = 0; i < config.row_count; i += 2 ){
    ...
    assert: function(config, test_return){
        ...
        if( tds.length != config.row_count / 2 ) throw("tds.length = " + tds.length);
