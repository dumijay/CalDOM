_pfreak.tasks.push({
    ...
    short_name: "set_attr_multiple_elems",

    category: "unit_level_multiple_elements",
    description: 'Setting attribute of multiple elements. (10,000 repeats).',
    ...
    candidates: {
        "vanilla_js": function(config){
            ...
            for( var elem_i = 0, len = config.vanillaInstance.length; elem_i < len; elem_i++ ){
                config.vanillaInstance[elem_i].setAttribute("type", "checkbox");
            }
        ...
        "caldom": function(config){
            ...
            config.caldomInstance.attr("type", "checkbox");

        },

        "umbrellajs": function(config){

            config.umbrellaJSInstance.attr("type", "checkbox");

        },

        "zepto": function(config){
            config.zeptoInstance.attr("type", "checkbox");
        },

        "jquery": function(config){

            config.jqueryInstance.attr("type", "checkbox");
